...
class Multicache
{
    ...
    public function __construct($options = null)
    {
        ...
        $this->_options = array(
            'cachebase' => false !== $conf ? $conf->getC('cache_path', $c_path ): $c_path,
            'lifetime' => false !== $conf ? (int) $conf->getC('cachetime'): 3600,
        	'handler' => false !== $conf ? $conf->getC('cache_handler', 'fastcache') : 'file',//file /fastcache
            'storage' => false !== $conf ? $conf->getC('storage', 'fastcache') : 'fastcache',//only fastcache
            'defaultgroup' => 'default',
            'locking' => false,
            'locktime' => 15,
            'checkTime' => true,
            'caching' => false !== $conf && ($conf->getC('caching') >= 1) ? true : false
        );
