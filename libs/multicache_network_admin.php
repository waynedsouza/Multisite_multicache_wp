...
class MulticacheNetworkAdmin
{
	...
	public function getNetworkManagementObject()
	{
		$args = array(
				'network_id' => null,
				'public'     => null,
		...
		if ($this->count > $this->limit && $this->limit)
		{
			$array = array_slice($array, $this->start, $this->limit);
		}
		Return $array;
	}
	protected function getCount()
	{
		Return count(wp_get_sites( $args ));
	}
	public function getPagination()
	{
		...
		$paged = $this->getPaged();
		$this->start = $paged == 1 ? 0 : $this->limit*($paged -1);
		$this->count = !isset($this->count)? $this->getCount(): $this->count;
		$this->total_pages = !empty($this->limit) && !empty($this->count)? (int) ceil($this->count / $this->limit):1;
		...
		$pagination = array('total_pages' => $this->total_pages,'current_page' => $this->paged);
	Return 	$pagination;
	}
	...
	public static function getInstance()
	{
		...
		if (empty(self::$instance))
		{
			...
			self::$instance = new MulticacheNetworkAdmin();
		}
	...
	protected function checkMultisiteConfigExists($path)
	{
		if(empty($path)){
			Return false;
		}
		$end_tag = substr($path,-1 , 1);
		$path = $end_tag ==='/'? $path.'multicache_config.php':$path.'/'.'multicache_config.php';
		$obj = new stdClass();
		$obj->path = $path;
		if(is_file($path))
		{
	...
	public function classifyBlog($site_id , $blog_id )
	{
	Return $this->updateBlogsActivationStatus($site_id , $blog_id ,  null , false);	
	}
	protected function updateBlogsActivationStatus($site_id , $blog_id , $activation_object = null , $activate = true)
	{
		$blog_info = get_blog_details( $blog_id );
		$current_blog_path = $blog_info->path;
		$strat_dir = dirname(__FILE__). '/multisite_strategy/';
		if(!is_dir($strat_dir))
		{
			MulticacheHelper::createMultisiteDir($strat_dir);
		}
		//case 1: subdomain install
		if( defined('SUBDOMAIN_INSTALL') && SUBDOMAIN_INSTALL===true)
		{
			...
			$expected_path = dirname(__FILE__). '/multisite_strategy/'.$site;
			if(!is_dir($expected_path))
			{
				MulticacheHelper::createMultisiteDir($expected_path);
			}
				
			$expected_path = $expected_path .'/'. $blog.'/';
			if(!is_dir($expected_path))
			{
				MulticacheHelper::createMultisiteDir($expected_path);
			}
			$s1 = 'S'.$site;
			$expected_config_namespace =  $s1.'Bl'.preg_replace('~[^a-zA-Z0-9]~','',$blog);
			$expected_config_namespace = 'MulticacheConfig'.$expected_config_namespace;
			$success = $this->updateMultisiteConfig($expected_path , $expected_config_namespace , $activation_object);
			Return $success;
				
		}
		//case subfolder install
		if(defined('PATH_CURRENT_SITE')  && '/' !== PATH_CURRENT_SITE)
		{
			//$this->resolveForSubFolders($site_id , $blog_id , $activation_object);
		...
		//normal folder install
		
		$domain = $blog_info->domain;
		$current_site_domain = str_replace(array('www.','.'),'',$domain);
		$current_site_domain = preg_replace('~[^a-zA-Z0-9]~','',$current_site_domain);
		
		$expected_path = dirname(__FILE__). '/multisite_strategy/'.$current_site_domain;
		if(!is_dir($expected_path))
		{
			MulticacheHelper::createMultisiteDir($expected_path);
		}
		$b_path = $current_blog_path==='/'? '/'.'main/' :  $current_blog_path;
		$expected_path = $expected_path . $b_path;
		//$expected_config_namespace = 'S'.$current_site_domain.'Bl'.
		$s1 = 'S'.$current_site_domain;
		$expected_config_namespace = $current_blog_path==='/'? $s1.'Blmain': $s1.'Bl'.preg_replace('~[^a-zA-Z0-9]~','',$current_blog_path);
		$expected_config_namespace = 'MulticacheConfig'.$expected_config_namespace;
		if(!is_dir($expected_path))
		{
			MulticacheHelper::createMultisiteDir($expected_path);
	...
	protected function updateMultisiteConfig($path , $config_namespace , $blog)
	{
		
		if(empty($path) || empty($config_namespace))
		{
			
			Return false;
		}
		
		$config_path = $this->checkMultisiteConfigExists($path);
		
		if(!empty($config_path->exists))
		{
			require_once $config_path->path;
			
			$obj = new $config_namespace;
		}
		else {
			$obj = new stdClass();
		}
		$path = $config_path->path;
		$obj->network_activate = $blog['activate'];
		$obj->network_caching = $blog['cache'];
		$obj->network_optimize = $blog['optimize'];
		$success = MulticacheHelper::writeToConfigMultisite($obj, $path ,$config_namespace );
		Return $success;
		
	}
	public function processNetworkTasks($obj)
	{
		if(empty($obj))
		{
			Return false;
		}
		foreach($obj As $site_id =>$blog_obj)
		{
			if(!empty($blog_obj))
			{
				foreach($blog_obj AS $blog_id => $blog)
				{
				$r = update_blog_option ($blog_id, 'multicache_network_config',$blog, true);
				$this->updateBlogsActivationStatus($site_id , $blog_id , $blog);
				}
			}
		}
	}
